...
const riskFeatureExtractor_1 = require("./riskFeatureExtractor");
class RiskCalculator {
    static calculateRisk(request) {
        const features = riskFeatureExtractor_1.RiskFeatureExtractor.extractFeatures(request);
        const categories = ['security', 'performance', 'correctness', 'architecture', 'maintainability'];
        const categoryRisks = [];
        ...
        categories.forEach((cat) => {
            ...
            if (findingCount > 0) {
                const catFindings = (request.findings || []).filter((f) => (f.category || '').toLowerCase() === cat);
            ...
            const hasHistoricalRule = (request.historicalRules || []).some((hr) => (hr.type || '').toLowerCase() === cat);
        ...
        if (overallRiskPercent >= 85)
            overallRiskLevel = 'CRITICAL';
        else if (overallRiskPercent >= 65)
            overallRiskLevel = 'HIGH';
        else if (overallRiskPercent >= 35)
        ...
        const primaryFactors = [
            {
                ...
                color: overallRiskLevel === 'HIGH' || overallRiskLevel === 'CRITICAL' ? 'red' : 'amber',
            },
            {
                id: 'f-2',
        ...
        const summary = `${overallRiskLevel} risk forecast for ${request.fileName} (${request.language.toUpperCase()}). ${features.isPersonalized
