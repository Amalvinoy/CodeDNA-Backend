...
class JavaScriptAnalyzer {
    language = 'javascript';
    async analyze(sourceCode, fileName = 'file.js') {
        const findings = [];
        ...
        const requestAuthVars = new Map();
        // Pass 1: Identify variables assigned directly from untrusted request auth parameters
        lines.forEach((line, index) => {
            const trimmed = line.trim();
            if (trimmed.startsWith('//') || trimmed.startsWith('*') || trimmed.startsWith('/*'))
                return;
            const reqAuthAssignMatch = trimmed.match(/\b(?:const|let|var)\s+([a-zA-Z0-9_$]+)\s*=\s*(req\.(?:query|body|params|headers)\.(?:admin|isAdmin|role|roleName|group|isSuperUser|superuser|accessLevel|permission))\b/i);
        ...
        lines.forEach((line, index) => {
            ...
            if (/\beval\s*\(/.test(trimmed)) {
                ...
                findings.push({
                    ...
                    description: 'Use of eval() executes strings as JavaScript code, allowing attackers to inject malicious code and bypass security scopes.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'eval() opens severe remote code execution vulnerabilities and disables JavaScript V8 engine JIT optimizations.',
                    suggestedFix: 'Parse data safely using JSON.parse() or replace dynamic code evaluation with structured map/object lookups.',
                    confidence: 0.98,
            ...
            if (/(`SELECT|`INSERT|`UPDATE|`DELETE|`DROP|`ALTER|"\s*SELECT|'\s*SELECT).*\$\{/i.test(trimmed)) {
                rawSqlFound = true;
                findings.push({
                    ...
                    description: 'Unsanitized parameters interpolated directly into SQL string templates allow query manipulation and unauthorized data exfiltration.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'SQL injection is an OWASP Top 10 critical risk that can lead to complete database compromise.',
                    suggestedFix: 'Use parameterized queries (e.g., $1, ? placeholders) provided by your query builder or ORM.',
                    confidence: 0.95,
                    codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                });
            }
            ...
            if ((hasSqlKeywords || isDbQueryCall) && isSqlConcatPattern) {
                ...
                findings.push({
                    ...
                    description: 'Dynamic concatenation of untrusted variables into a SQL query string allows attackers to manipulate query logic and execute arbitrary database operations.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'SQL injection is an OWASP Top 10 critical security risk that can lead to complete database compromise, data tampering, or unauthorized data exfiltration.',
                    suggestedFix: 'Use parameterized/prepared queries with placeholders (e.g. `?` or `$1`) and pass dynamic inputs as separate parameters array.',
                    confidence: 0.96,
                    codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                });
            }
            ...
            if (credMatch) {
                ...
                if (!isEnvOrConfig && !isTypePassword && !isPlaceholder && val.length >= 4) {
                    ...
                    findings.push({
                        ...
                        description: `Potential hardcoded credential or secret ('${credMatch[1]}') discovered directly in source code.`,
                        lineStart: lineNum,
                        lineEnd: lineNum,
                        codeSnippet: trimmed,
                        whyItMatters: 'Hardcoded credentials committed to version control can be discovered by attackers, leading to unauthorized access, privilege escalation, and data breaches.',
                        suggestedFix: 'Extract credentials to environment variables (e.g., `process.env.DB_PASSWORD`) or a secure secrets management system.',
                        confidence: 0.94,
                        codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                    });
                }
            ...
            const directReqAuthMatch = trimmed.match(/if\s*\([^)]*req\.(?:query|body|params)\.(admin|isAdmin|role|isSuperUser|superuser|group)\s*===?\s*["'][^"']+["']/i);
            ...
            if (reqAuthMatchFound) {
                ...
                findings.push({
                    ...
                    description: `Authorization check relies directly on client-controlled request input (${matchedAuthSource}).`,
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'Allowing client-controlled query parameters or request body values to determine administrative access causes Broken Access Control (OWASP Top 10), enabling privilege escalation.',
                    suggestedFix: 'Derive authorization decisions exclusively from trusted server-side identity (e.g., cryptographically verified session or verified JWT tokens).',
                    confidence: 0.93,
                    codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                });
            }
            ...
            if (hasErrorDetail) {
                ...
                if (isResponseCall && !isLoggingCall) {
                    ...
                    findings.push({
                        ...
                        description: 'Internal exception details (message or stack trace) are returned directly in the HTTP client response.',
                        lineStart: lineNum,
                        lineEnd: lineNum,
                        codeSnippet: trimmed,
                        whyItMatters: 'Exposing raw error messages or stack traces leaks database schema, internal paths, and implementation details to potential attackers (CWE-209).',
                        suggestedFix: 'Return a generic, client-safe error message (e.g., `res.status(500).json({ error: "Internal server error" })`) and log detailed diagnostics securely on the server.',
                        confidence: 0.92,
                        codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                    });
                }
            ...
            if (/[^!=]==[^=]/.test(trimmed) && !trimmed.startsWith('//') && !trimmed.includes('typeof')) {
                ...
                if (looseEqualityCount <= 2) {
                    findings.push({
                        ...
                        description: 'Loose equality (==) performs unexpected type coercion, causing subtle logical bugs when comparing falsy values.',
                        lineStart: lineNum,
                        lineEnd: lineNum,
                        codeSnippet: trimmed,
                        whyItMatters: 'Type coercion can result in `0 == ""` evaluating to true and bypassing validation guards.',
            ...
            if (/\bvar\s+[a-zA-Z_$][a-zA-Z0-9_$]*/.test(trimmed)) {
                ...
                if (varDeclarationCount <= 3) {
                    findings.push({
                        ...
                        description: "Variables declared with 'var' are function-scoped rather than block-scoped and are hoisted, creating potential scoping and shadowing bugs.",
                        lineStart: lineNum,
                        lineEnd: lineNum,
                        codeSnippet: trimmed,
                        whyItMatters: "'var' lacks block scoping in loops and conditionals, which can inadvertently overwrite outer variables or cause race conditions in asynchronous callbacks.",
                        suggestedFix: "Replace 'var' with 'const' for identifiers that are not reassigned, or 'let' for loop counters and mutable bindings.",
                        confidence: 0.95,
                        codeContextSnippet: (0, contextSnippetHelper_1.generateCodeContextSnippet)(sourceCode, lineNum),
                    });
                }
            ...
            if (/\.innerHTML\s*=/.test(trimmed)) {
                findings.push({
                    ...
                    description: 'Assigning untrusted or dynamic strings directly to innerHTML can execute arbitrary client-side scripts.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'XSS vulnerabilities allow attackers to hijack user sessions and steal authentication tokens.',
