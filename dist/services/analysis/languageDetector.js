...
class LanguageDetector {
    ...
    static normalizeLanguage(input) {
        if (!input)
            return null;
    ...
    static getSupportedLanguagesList() {
        return [
            'javascript',
            'typescript',
            'python',
     ...
     */
    static validateLanguageWithCode(selectedLanguage, sourceCode) {
        ...
        let pythonScore = 0;
        if (/\bdef\s+[a-zA-Z_]\w*\s*\([^)]*\)\s*:/.test(stripped))
            pythonScore += 3;
        if (/\bclass\s+[a-zA-Z_]\w*(\s*\([^)]*\))?\s*:/.test(stripped))
            pythonScore += 3;
        if (/\belif\b/.test(stripped))
            pythonScore += 3;
        if (/\bself\.\w+/.test(stripped))
            pythonScore += 2;
        if (/\bfrom\s+[a-zA-Z_]\w*\s+import\s+[a-zA-Z_]\w+/.test(stripped))
            pythonScore += 3;
        if (/\bimport\s+[a-zA-Z_]\w+(\s+as\s+\w+)?\s*(?:$|\n)/m.test(stripped))
            pythonScore += 2;
        if (/\bexcept(\s+[a-zA-Z_]\w*)?(\s+as\s+\w+)?\s*:/.test(stripped))
            pythonScore += 3;
        if (/\b(?:None|True|False)\b/.test(stripped))
            pythonScore += 1;
        if (/\bpass\b/.test(stripped))
            pythonScore += 2;
        let jsTsScore = 0;
        if (/\b(?:const|let|var)\s+[a-zA-Z_$]\w*\s*=/.test(stripped))
            jsTsScore += 3;
        if (/\bfunction\s*[a-zA-Z_$]*\s*\([^)]*\)\s*\{/.test(stripped))
            jsTsScore += 3;
        if (/=>\s*\{?/.test(stripped))
            jsTsScore += 3;
        if (/console\.(?:log|error|warn|info)\s*\(/.test(stripped))
            jsTsScore += 3;
        if (/\b(?:export\s+default|module\.exports\s*=|require\s*\()/.test(stripped))
            jsTsScore += 3;
        if (/\bimport\s+.*?\s+from\s+/.test(stripped))
            jsTsScore += 2;
        if (/===|!==/.test(stripped))
            jsTsScore += 2;
        let sqlScore = 0;
        if (/\bSELECT\s+.*?\s+FROM\b/i.test(stripped))
            sqlScore += 4;
        if (/\bINSERT\s+INTO\s+.*?\s+VALUES\b/i.test(stripped))
            sqlScore += 4;
        if (/\bUPDATE\s+.*?\s+SET\b/i.test(stripped))
            sqlScore += 4;
        if (/\bDELETE\s+FROM\b/i.test(stripped))
            sqlScore += 4;
        if (/\bCREATE\s+TABLE\b/i.test(stripped))
            sqlScore += 4;
        if (/\bALTER\s+TABLE\b|\bDROP\s+TABLE\b/i.test(stripped))
            sqlScore += 4;
        let javaScore = 0;
        if (/public\s+static\s+void\s+main\s*\(/.test(stripped))
            javaScore += 5;
        if (/System\.out\.(?:println|print)\s*\(/.test(stripped))
            javaScore += 4;
        if (/public\s+(?:class|interface|enum)\s+[a-zA-Z_]\w*/.test(stripped))
            javaScore += 3;
        if (/package\s+[a-zA-Z_]\w*(?:\.[a-zA-Z_]\w*)*\s*;/.test(stripped))
            javaScore += 3;
        if (/@Override\b/.test(stripped))
            javaScore += 2;
        let goScore = 0;
        if (/^package\s+[a-zA-Z_]\w*/m.test(stripped))
            goScore += 3;
        if (/func\s+(?:\([^)]+\)\s+)?[a-zA-Z_]\w*\s*\([^)]*\)/.test(stripped))
            goScore += 3;
        if (/:=/.test(stripped))
            goScore += 2;
        if (/fmt\.(?:Println|Printf|Print|Sprintf)\b/.test(stripped))
            goScore += 3;
        let rustScore = 0;
        if (/fn\s+[a-zA-Z_]\w*\s*\([^)]*\)/.test(stripped))
            rustScore += 3;
        if (/let\s+mut\s+/.test(stripped))
            rustScore += 3;
        if (/impl\s+[a-zA-Z_]\w*/.test(stripped))
            rustScore += 3;
        if (/println!\s*\(|format!\s*\(/.test(stripped))
            rustScore += 3;
        let cppScore = 0;
        if (/#include\s*<[a-zA-Z0-9_./]+>/.test(stripped))
            cppScore += 4;
        if (/std::|cout\s*<<|cin\s*>>/.test(stripped))
            cppScore += 4;
        if (/int\s+main\s*\(\s*(?:void|int\s+argc)?/.test(stripped))
            cppScore += 3;
        if (/printf\s*\(|scanf\s*\(/.test(stripped))
            cppScore += 2;
        let phpScore = 0;
        if (/<\?php|\?>/.test(stripped))
            phpScore += 5;
        if (/\$[a-zA-Z_]\w*\s*=/.test(stripped))
            phpScore += 3;
        if (/echo\s+/.test(stripped))
            phpScore += 2;
