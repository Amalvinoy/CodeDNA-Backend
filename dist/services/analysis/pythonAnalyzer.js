...
class PythonAnalyzer {
    language = 'python';
    async analyze(sourceCode, fileName = 'script.py') {
        const findings = [];
        ...
        lines.forEach((line, index) => {
            ...
            if (/\b(eval|exec)\s*\(/.test(trimmed) && !trimmed.startsWith('#')) {
                ...
                findings.push({
                    ...
                    description: 'Dynamic execution of arbitrary Python strings introduces remote code execution vulnerabilities.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'Attackers can execute arbitrary system commands and access internal environment variables.',
            ...
            if (/def\s+\w+\s*\(.*=\s*(\[\]|\{\})\s*\):?/.test(trimmed) && !trimmed.startsWith('#')) {
                ...
                findings.push({
                    ...
                    description: 'Default arguments in Python are evaluated once at function definition time, not on each invocation.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'Mutations to the list or dictionary persist across all future calls, creating shared state bugs.',
            ...
            if (/^except\s*:/.test(trimmed) && !trimmed.startsWith('#')) {
                findings.push({
                    ...
                    description: 'A bare `except:` catches `KeyboardInterrupt` and `SystemExit`, making it difficult to terminate the program.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'Hides critical system errors and makes debugging unexpected runtime exceptions significantly harder.',
            ...
            if (/f["'].*(SELECT|INSERT|UPDATE|DELETE).*\{/i.test(trimmed) && !trimmed.startsWith('#')) {
                ...
                findings.push({
                    ...
                    description: 'Constructing SQL statements with f-string interpolation allows direct SQL injection vulnerabilities.',
                    lineStart: lineNum,
                    lineEnd: lineNum,
                    codeSnippet: trimmed,
                    whyItMatters: 'User parameters can break out of string literals and execute arbitrary database queries.',
