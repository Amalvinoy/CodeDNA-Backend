...
exports.WeaknessAnalyzer = void 0;
const patternNormalizer_1 = require("./patternNormalizer");
class WeaknessAnalyzer {
    static analyzePatternsAndWeaknesses(reviews) {
        ...
        const sortedReviews = [...reviews].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        ...
        pastReviews.forEach((review) => {
            const revId = review.reviewIdString || review._id?.toString() || 'REV';
            const revDate = new Date(review.createdAt || Date.now());
            (review.findings || []).forEach((finding) => {
                const normalized = patternNormalizer_1.PatternNormalizer.normalizeFinding(finding);
                ...
                if (revDate < item.firstDetectedAt)
                    item.firstDetectedAt = revDate;
                if (revDate > item.lastDetectedAt)
                    item.lastDetectedAt = revDate;
            });
        });
        // Scan recent reviews
        ...
        patternMap.forEach((p) => {
            ...
            if (p.occurrenceCount >= 2) {
                ...
                const priority = p.severity === 'critical' || p.occurrenceCount >= 4
                    ? 'High Priority'
...
exports.WeaknessAnalyzer = WeaknessAnalyzer;
//# sourceMappingURL=weaknessAnalyzer.js.map