...
class AchievementService {
     ...
     */
    static calculateStreak(reviews) {
        if (!reviews || reviews.length === 0)
            return 0;
        // Extract unique calendar dates in YYYY-MM-DD UTC
        const uniqueDates = Array.from(new Set(reviews.map((r) => new Date(r.createdAt).toISOString().split('T')[0]))).sort().reverse(); // newest first
     ...
     */
    static evaluateBadges(reviews) {
        const totalReviews = reviews.length;
        // Detect real security improvements: 0 critical issues and security metric >= 9.0 or quality >= 8.5
        const hasSecurityImprovement = reviews.some((r) => (r.criticalCount === 0 && (r.qualityScore || 0) >= 8.5) ||
        ...
        const hasPerformanceImprovement = reviews.some((r) => (r.metrics?.performance !== undefined && r.metrics.performance >= 9.0) ||
     ...
     */
    static evaluateMilestones(reviews) {
        const totalReviews = reviews.length;
        const cleanReviewsCount = reviews.filter((r) => (r.criticalCount === 0 && r.warningCount === 0) || (r.qualityScore || 0) >= 9.0).length;
        return [
            {
                id: 'm-1',
    ...
    static async getAchievements(userId) {
        let achievement = await models_1.Achievement.findOne({ userId });
        if (!achievement) {
     ...
     */
    static async recordReviewCompletion(userId, reviewDoc) {
        const isClean = (reviewDoc.criticalCount === 0 && reviewDoc.warningCount === 0) ||
            (reviewDoc.qualityScore !== undefined && reviewDoc.qualityScore >= 9.0);
        const xpEarned = 100 + (isClean ? 50 : 0);
        const currentDoc = await models_1.Achievement.findOne({ userId });
        const currentXp = (currentDoc?.currentXp || 0) + xpEarned;
        const allReviews = await models_1.Review.find({ userId, status: 'completed' }).sort({ createdAt: -1 });
        ...
        return await models_1.Achievement.findOneAndUpdate({ userId }, {
            $set: {
                currentXp,
     ...
     */
    static async recalculateUserAchievements(userId) {
        const reviews = await models_1.Review.find({ userId, status: 'completed' }).sort({ createdAt: -1 });
        // Calculate XP from all completed reviews
        let totalXp = 0;
        for (const rev of reviews) {
            const isClean = (rev.criticalCount === 0 && rev.warningCount === 0) ||
