...
exports.HistoricalRetriever = void 0;
const models_1 = require("../../models");
const embeddings_1 = require("../embeddings");
const env_1 = require("../../config/env");
class HistoricalRetriever {
    static DEFAULT_THRESHOLD = 0.65;
    static DEFAULT_TOP_K = 3;
    static async retrieveRelevantRules(params) {
        const limit = Math.max(1, Math.min(5, params.limit || this.DEFAULT_TOP_K));
        const threshold = params.threshold ??
            (env_1.env.HISTORICAL_MATCH_THRESHOLD
                ? parseFloat(env_1.env.HISTORICAL_MATCH_THRESHOLD)
                : this.DEFAULT_THRESHOLD);
        try {
            // 1. Fetch all indexed historical rules from MongoDB
            const allRules = (await models_1.HistoricalRule.find({}));
            ...
            const queryVector = await embeddings_1.EmbeddingService.generateEmbedding(queryText);
            // 4. Compute similarity scores across all rules
            const scoredRules = [];
            for (const rule of allRules) {
                let similarity = 0;
                if (rule.embedding && rule.embedding.length > 0) {
                    similarity = embeddings_1.EmbeddingService.cosineSimilarity(rule.embedding, queryVector);
                }
                else {
                ...
                const matchesFindingCategory = (params.findings || []).some((f) => (f.category || '').toLowerCase() === ruleType);
            ...
            const matches = topMatches.map((item) => ({
                ruleId: item.rule._id?.toString() || item.rule.externalId,
...
exports.HistoricalRetriever = HistoricalRetriever;
//# sourceMappingURL=historicalRetriever.js.map