...
class HistoricalIngestionService {
    static async importFromCSVFile(filePath) {
    ...
    static async importFromCSVContent(csvContent) {
        const startTime = Date.now();
        const report = {
            totalRows: 0,
            validRows: 0,
        ...
        if (idIndex === -1 || typeIndex === -1 || descIndex === -1) {
            throw new Error(`Invalid CSV header. Expected "id,type,description" but received: "${headerLine}"`);
        }
        ...
        for (const record of validRecords) {
            try {
                ...
                const existing = await models_1.HistoricalRule.findOne({ externalId: record.id });
                // Generate embedding if new or changed
                let embedding = existing?.embedding;
                if (!embedding || embedding.length === 0 || existing.description !== record.description) {
                    embedding = await embeddings_1.EmbeddingService.generateEmbedding(`${record.type}: ${record.description}`);
                }
                const categoryTag = record.type.toUpperCase();
                const updateResult = await models_1.HistoricalRule.updateOne({ externalId: record.id }, {
                    $set: {
...
//# sourceMappingURL=historicalIngestion.service.js.map