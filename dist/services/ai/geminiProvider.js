...
class GeminiProvider {
    ...
    constructor() {
        ...
        this.model = env_1.env.GEMINI_MODEL || 'gemini-3.6-flash';
    }
    isAvailable() {
    ...
    normalizeReviewOutput(parsed) {
        ...
        const findings = rawFindings.map((f, idx) => {
            ...
            if (!validCategories.has(category)) {
                ...
                else if (category.includes('arch'))
                    category = 'architecture';
                else if (category.includes('maint') || category.includes('read'))
            ...
            if (!validSeverities.has(severity)) {
                if (severity.includes('crit') || severity.includes('fatal') || severity.includes('block'))
                    ...
                    severity = 'low';
                else if (severity.includes('info') || severity.includes('note'))
                    severity = 'info';
                else
                    severity = 'medium';
            }
            const lineStart = Math.max(1, parseInt(f.lineStart || f.startLine || f.line || '1', 10) || 1);
            const lineEnd = Math.max(lineStart, parseInt(f.lineEnd || f.endLine || f.line || lineStart, 10) || lineStart);
            return {
                category,
                severity,
                ...
                whyItMatters: (f.whyItMatters ||
                    f.impact ||
                    f.explanation ||
                    'May impact software security, performance, or correctness.').trim(),
                suggestedFix: (f.suggestedFix ||
                    f.fix ||
                    f.recommendation ||
                    'Refactor source code according to safe engineering practices.').trim(),
                ...
                historicalSimilarity: typeof f.historicalSimilarity === 'number' ? f.historicalSimilarity : undefined,
            };
        });
        ...
        const metrics = {
            ...
            maintainability: typeof m.maintainability === 'number' ? Math.max(1, Math.min(10, m.maintainability)) : 8.5,
        };
        return {
            summary,
            findings,
    ...
    normalizeOptimizationOutput(parsed) {
        ...
        const changes = rawChanges.length > 0
            ? rawChanges.map((c) => ({
...
`;
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.model}:generateContent?key=${this.apiKey}`, {
                ...
                body: JSON.stringify({
                    systemInstruction: {
                        parts: [{ text: reviewPrompt_1.SYSTEM_REVIEW_PROMPT }],
                    },
            ...
            let validated;
            try {
                validated = reviewPrompt_1.aiResponseSchema.parse(normalized);
            }
            catch (e) {
            ...
            console.log(`[GeminiProvider] Code analysis succeeded with ${this.model} in ${Date.now() - startTime}ms (${findings.length} findings)`);
            return {
...
`;
            const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${this.model}:generateContent?key=${this.apiKey}`, {
                ...
                body: JSON.stringify({
                    systemInstruction: {
                        parts: [{ text: optimizationPrompt_1.SYSTEM_OPTIMIZATION_PROMPT }],
                    },
                    contents: [{ role: 'user', parts: [{ text: userPrompt }] }],
                    generationConfig: {
                        responseMimeType: 'application/json',
            ...
            let validated;
            try {
                validated = optimizationPrompt_1.aiOptimizationResponseSchema.parse(normalized);
            }
            catch (e) {
                throw new Error(`Gemini optimization response failed schema validation: ${e.message}`);
            }
            console.log(`[GeminiProvider] Code optimization succeeded with ${this.model} in ${Date.now() - startTime}ms`);
            return {
            ...
            console.warn(`[GeminiProvider] Code optimization failed after ${Date.now() - startTime}ms: ${err.message}`);
            throw err;
        }
        finally {
            clearTimeout(timeoutId);
...
`;
            ...
            console.log(`[GeminiProvider] Defense evaluation succeeded with ${this.model} in ${Date.now() - startTime}ms`);
            return {
            ...
            console.warn(`[GeminiProvider] Defense evaluation failed after ${Date.now() - startTime}ms: ${err.message}`);
            throw err;
        }
        finally {
            clearTimeout(timeoutId);
