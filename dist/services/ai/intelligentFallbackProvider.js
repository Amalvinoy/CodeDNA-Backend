...
exports.IntelligentFallbackProvider = void 0;
class IntelligentFallbackProvider {
    name = 'CodeDNA Grounded Semantic Heuristic Engine';
    isAvailable() {
        return true; // Always available
    }
    async analyzeCode(request) {
        const findings = [...(request.staticFindings || [])];
        ...
        if (request.historicalRules && request.historicalRules.length > 0) {
            findings.forEach((finding) => {
                const matchingRule = request.historicalRules?.find((hr) => {
                    ...
                    const descMatch = finding.description.toLowerCase().includes('sql') ||
        ...
        if (hasCritical) {
            ...
            summary = `Critical vulnerability detected in ${request.fileName} (${request.language.toUpperCase()}): ${topCritical?.title || 'Security/Correctness hazard'}. ${historicalRulesCount > 0
...
exports.IntelligentFallbackProvider = IntelligentFallbackProvider;
//# sourceMappingURL=intelligentFallbackProvider.js.map