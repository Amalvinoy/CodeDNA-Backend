...
exports.seedInitialDatabaseData = void 0;
...
const seedInitialDatabaseData = async () => {
    try {
        // 1. Seed User
        let user = await models_1.User.findOne({ email: 'engineer@codedna.dev' });
        if (!user) {
        ...
        // 2. Seed CodeDNA
        const existingDna = await models_1.CodeDNA.findOne({ userId: defaultUserId });
        if (!existingDna) {
            await models_1.CodeDNA.create({
                ...
                strengths: [
                    {
                        ...
                        description: 'Zero critical OWASP vulnerabilities introduced in last 40 commits. Robust input validation patterns detected.',
                        color: 'green',
                    },
                    {
                        id: 'str-2',
                        ...
                        description: 'Consistent separation of concerns. High adherence to clean architecture principles across modules.',
                        color: 'blue',
                    },
                ],
                areasForOptimization: [
                    {
                        ...
                        description: 'N+1 query patterns detected in UserService.ts. Redundant data fetching increasing latency overhead by ~12%.',
                    },
                ],
                evolutionTrajectory: [
                    { month: 'Mar', score: 62 },
        ...
        const reviewCount = await models_1.Review.countDocuments();
        if (reviewCount === 0) {
            await models_1.Review.create([
                {
                    userId: defaultUserId,
  ...
  try {
    ...
    return res.status(500).json({ error: err.message });
                    ...
                    qualityScore: 7.8,
                    scoreDelta: 0.9,
                    aiSummary: 'The implementation introduces a major security flaw in the user login query structure. While performance is optimized, the current string interpolation method exposes the database to injection attacks. Immediate remediation required.',
                    issuesCount: 3,
                    criticalCount: 1,
                    warningCount: 1,
                    findings: [
                        {
                            ...
                            description: 'Direct interpolation of unsanitized input into SQL query string.',
                            lineStart: 24,
                            lineEnd: 24,
                            codeSnippet: "const query = `SELECT * FROM users WHERE username = '${username}' AND password = '${password}'`;",
                            whyItMatters: 'Unsanitized user input is being directly interpolated into a raw SQL query string. This allows malicious actors to manipulate the query logic, potentially extracting unauthorized data or bypassing authentication.',
                            suggestedFix: 'Refactor the raw query to use parameterized queries (prepared statements) provided by the ORM, separating the SQL logic from the user-supplied data variables.',
                            confidence: 0.95,
                        ...
                        },
                        {
                            ...
                            description: 'Fetching associated roles inside a loop causes multiple round-trips to the database.',
                            lineStart: 87,
                            lineEnd: 87,
                            codeSnippet: 'for (const user of users) { user.roles = await db.roles.find({ userId: user.id }); }',
                            whyItMatters: 'Fetching associated roles inside a loop causes multiple round-trips to the database, resulting in exponential latency under load.',
                            suggestedFix: 'Eager load the relations in a single join query or batch with WHERE IN / populate.',
                            confidence: 0.88,
                        ...
                        },
                        {
                            ...
                            description: 'Non-standard camelCase identifier found where PascalCase interface convention is established.',
                            lineStart: 12,
                            lineEnd: 12,
                            codeSnippet: 'export interface user_payload { id: string; }',
                            whyItMatters: 'Non-standard camelCase identifier found where PascalCase interface convention is established across the repository.',
    ...
    return [r.strip().upper() for r in records if r]`,
                    ...
                    qualityScore: 9.6,
                    scoreDelta: 0.4,
                    aiSummary: 'Data transformation pipeline is well structured and follows Pythonic conventions. No critical risks found.',
                    issuesCount: 0,
                    criticalCount: 0,
                    warningCount: 0,
                    findings: [],
...
type Pool struct {
    ...
    ch chan int
                    ...
                    qualityScore: 8.8,
                    scoreDelta: -0.2,
                    aiSummary: 'Worker pool implementation has proper mutex locking, but channel close could panic if cancelled concurrently.',
                    issuesCount: 1,
                    criticalCount: 0,
                    warningCount: 1,
                    findings: [],
        ...
        const rulesCount = await models_1.HistoricalRule.countDocuments();
        if (rulesCount === 0) {
            const csvPath = path_1.default.resolve(__dirname, '../../data/historical_rules.csv');
            if (fs_1.default.existsSync(csvPath)) {
                const report = await historical_1.HistoricalIngestionService.importFromCSVFile(csvPath);
        ...
        const existingRisk = await models_1.RiskPrediction.findOne({ userId: defaultUserId });
        if (!existingRisk) {
            await models_1.RiskPrediction.create({
                userId: defaultUserId,
                ...
                riskPercent: 82,
                riskLevel: 'HIGH',
                recommendation: 'Immediate intervention recommended before merge to prevent regression.',
                ...
                diagnosticReasoning: {
                    summary: 'The prediction engine has identified a structural pattern in the proposed changes to the EventHandler class that strongly correlates with runtime performance degradation.',
        ...
        // 6. Seed CodeBattle
        const existingBattle = await models_1.CodeBattle.findOne({ userId: defaultUserId });
        if (!existingBattle) {
            await models_1.CodeBattle.create({
                userId: defaultUserId,
...
export const processData = (items: DataItem[]): DataItem[] => {
  return items.reduce((acc, item) => {
    ...
    return acc;
                ...
                winnerExplanation: 'AI Optimized eliminates in-loop shallow object cloning, uses strict typing with DataItem, and leverages stream reduction for 3.2x faster execution in benchmark tests.',
            });
            console.log('🌱 Seeded default CodeBattle.');
        }
        // 7. Seed Real Achievements based on actual review data
        const existingAchievement = await models_1.Achievement.findOne({ userId: defaultUserId });
        if (!existingAchievement) {
            const { AchievementService } = await Promise.resolve().then(() => __importStar(require('../services/achievement.service')));
...
exports.seedInitialDatabaseData = seedInitialDatabaseData;
